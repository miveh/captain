...
List<RolesModel> rolesList = [
  ...
  RolesModel(
    ...
    options: [
      ...
      'اگر به نقش هایی که استفاده از آنها اجباریست دسترسی پیدا کند باید بر شخص دیگری اعمال کند مثل: دکتر، گادفادر، ناتاشا و ... . اگر نقشی را پیدا کند که اجباری بر آن نیست می‌تواند استفاده نکند مثل: تک تیرانداز، تفنگدار و...',
      ...
      'در صورتی که بمبر را انتخاب کند ودر شب کشته شود اطرافیان آن هم با او کشته می‌شوند',
  ...
  RolesModel(
    ...
    options: [
      'اگر رویینتن در طول روز بازی به نامیرا بودن خود اشاره ای کند حتی غیر مستقیم توسط گرداننده بازی باید حذف شود',
      'رویینتن در اثر اتفاق های شهروندی از بازی حذف می‌شود مثل گلوله تک تیرانداز، انفجار بمب، مرگ ماسونی، بافت عنکبوت، یا حمله روانشناس و ...',
      'رویینتن از حمله مرد قوی جان سالم به در نمی‌برد',
      'رویینتن اگر توسط ساقی انتخاب شده باشد با هر تیر از بازی خارج می‌شود',
    ],
  ),
  RolesModel(
    id: 18,
  ...
  RolesModel(
    ...
    role: 1,
    isSelected: false.obs,
    description: 'اسنایپر شهروندی است که قابلیت شلیک در شب را دارد، هر شب بیدار میشود، به تعدادی که گرداننده می‌گوید تیر دارد و به بازیکنی که فکر می‌کند مافیاست شلیک میکند. تک تیرانداز ۱/۷ جعمیت شروع کننده بازی تیر دارد',
    options: [
      'تک تیرانداز در طول روز بازی شخصی را نمی‌تواند تهدید به کشتن کند در این صورت حذف می‌شود',
      'اگر فرمانده تیر تک تیرانداز را تأیید نکند تیر شلیک شده به خشاب تک تیرانداز برمی‌گردد',
      'اگر توسط ساقی انتخاب بشود و در همان شب به کسی شلیک کند در صورت تأیید فرمانده (شخصی که توسط تک تیرانداز انتخاب شده به فرمانده نشان داده میشود.) خودش کشته می‌شود',
    ],
  ),
  RolesModel(
    id: 22,
    ...
    role: 1,
    isSelected: false.obs,
    description: 'فرمانده شهروندی است که بعد از تک تیرانداز بیدار می‌شود، گرداننده هدف شلیک تک تیرانداز را به فرمانده اعلام میکند، فرمانده می‌تواند این شلیک را رد یا تأیید کند',
    options: [
  ...
  RolesModel(
    ...
    role: 1,
    isSelected: false.obs,
    description: 'بمبر شهروندی است که می‌تواند در طول روز تا قبل از آغاز رای گیری نقش خود را اعلام کند و بین دو بازیکن به اصطلاح منفجر شود و دو بازیکن را به همراه خود از بازی خارج کند. و در فاز شب اگر کشته شود اطرافیان خود هم با او کشته می‌شوند حتی اگر نجات دکتر را داشته باشند',
    options: [
      ' اگر ساقی بمبر را انتخاب کند یا بمبر ناتویی شده باشد در صورت اعلام نقش از بازی تنها حذف می‌شود',
      'اگر بمبر سایلنت شده باشد در فاز روز نمی‌تواند از قابلیت خود استفاده کند',
